...
auto expand_parrot(auto const& base_counts,
                   auto const& indices,
                   auto const& input_dims) {
    return input_dims.template replicate<2>(base_counts.gather(indices));
}
...
void test_clean_expand() {
    ...
    try {
        ...
        auto input_dims = fusion_array<
          typename thrust::device_vector<int>::iterator>(
          input_dims_vec->begin(), input_dims_vec->end(), input_dims_vec, {2, 5}
          // 2 dimensions, 5 elements each
        );
        ...
        std::cout << "❌ Exception in clean expand test: " << e.what()
                  << std::endl;
    }
}
